...
- `Phase::LookingAtBottomOfLibrary`, the `SearchingLibraryFor…` phases

During a library search, `previous` is a library `CardId` and the `legal`
list names `Action::TakeCard { card }` for specific library cards.
...
either library and their order". ADR 0095: a strategy reads a masked view,
...
2. **The `phase` field is a leak.** Sanitize `phase` in `PlayerView::of`
   ...
   the actor's. Add a test that an opponent view carries no library id.
