...
export default function CodexHome() {
  ...
  const play = () => {
    ...
    router.push(`/codex/play?a=${ea}&b=${eb}`);
  };
  const shuffle = () => {
    ...
    router.push(`/codex/play?a=${decks[i].key}&b=${decks[j].key}`);
  };
  ...
  return (
    <main className="mx-auto max-w-[720px] px-4 py-8">
      <header className="flex items-baseline justify-between">
        <h1 className="m-0 text-[20px]">sim — codex</h1>
        <Link href="/codex/preview" className="text-[13px] text-dim no-underline hover:text-text">
          Preview
        </Link>
      </header>
